...
export default function AdminLocalPacientes() {
  const { pacientes, pendientes, loading, saving, buscarPacientes, completarDatos } = usePacientes();
  ...
  const handleAbrirCompletar = (usuario) => {
    ...
    setFormCompletar({ fechaNacimiento: "", sexo: "", direccion: "", departamento: "", provincia: "", distrito: "" });
  ...
  const handleSubmitCompletar = async (e) => {
    e.preventDefault();
    const ok = await completarDatos(usuarioCompletar.idUsuario, formCompletar);
    ...
    <div style={{ padding: "2rem", background: "#f8fafc", minHeight: "100vh" }}>
      <style>
        {`
          ...
          .page-icon { width: 56px; height: 56px; border-radius: 16px; background: linear-gradient(135deg, #ea580c, #c2410c); display: flex; align-items: center; justify-content: center; box-shadow: 0 6px 16px rgba(234,88,12,0.35); flex-shrink: 0; }
          ...
          .avatar-mini-placeholder { width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, #ea580c, #c2410c); color: white; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 0.8rem; flex-shrink: 0; }
          ...
          .ubigeo-txt { color: #64748b; font-size: 0.84rem; }
          ...
          .modal-input:focus { outline: none; border-color: #ea580c; }
          .modal-row { display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; }
          ...
          .btn-save { background: linear-gradient(135deg, #ea580c, #c2410c); color: white; border: none; padding: 0.7rem 1.3rem; border-radius: 10px; font-weight: 700; cursor: pointer; box-shadow: 0 4px 12px rgba(234,88,12,0.3); }
      ...
      <div className="page-header">
        <div className="header-left">
          ...
          <div>
            ...
            <div className="page-subtitle">{pacientes.length} pacientes con datos completos</div>
          </div>
        </div>
      </div>
      ...
      {pendientes.length > 0 && (
        <div className="pendientes-box">
          <div className="pendientes-titulo">
            ...
            {pendientes.length} paciente(s) pendientes de completar datos
          </div>
      ...
      {pacientes.length === 0 ? (
        <div className="empty-state">
          <p style={{ fontSize: "1.1rem" }}>No se encontraron pacientes</p>
        </div>
      ) : (
        <div className="tabla-wrap">
          <div className="tabla-scroll">
            <table>
              ...
              <tbody>
                {pacientes.map((p) => (
                  <tr key={p.idPaciente || p.idUsuario}>
                    <td>
                    ...
                    <td className="ubigeo-txt">{[p.distrito, p.provincia, p.departamento].filter(Boolean).join(", ") || "N/A"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="modal-overlay">
          <div className="modal-box">
            <div className="modal-header">
              <div className="modal-title">Completar datos del paciente</div>
            ...
            <form onSubmit={handleSubmitCompletar}>
              <div className="modal-row">
                <div>
                  ...
                  <input
                    ...
                    value={formCompletar.fechaNacimiento}
                    onChange={(e) => setFormCompletar((prev) => ({ ...prev, fechaNacimiento: e.target.value }))}
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormCompletar((prev) => ({ ...prev, sexo: e.target.value }))}
                  >
              ...
              <input
                type="text"
                className="modal-input"
                value={formCompletar.direccion}
                onChange={(e) => setFormCompletar((prev) => ({ ...prev, direccion: e.target.value }))}
              ...
              <div className="modal-row">
                <div>
                  ...
                  <input
                    type="text"
                    className="modal-input"
                    value={formCompletar.departamento}
                    onChange={(e) => setFormCompletar((prev) => ({ ...prev, departamento: e.target.value }))}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    className="modal-input"
                    value={formCompletar.provincia}
                    onChange={(e) => setFormCompletar((prev) => ({ ...prev, provincia: e.target.value }))}
                  />
                </div>
              </div>
              ...
              <input
                type="text"
                className="modal-input"
                value={formCompletar.distrito}
                onChange={(e) => setFormCompletar((prev) => ({ ...prev, distrito: e.target.value }))}
              />
